...
gh repo create code-by-sia/homebrew-x --public \
  --description "Homebrew tap for the Xi toolchain"
git clone git@github.com:code-by-sia/homebrew-x.git
mkdir -p homebrew-x/Formula
cp packaging/homebrew/xi.rb homebrew-x/Formula/xi.rb
cd homebrew-x && git add Formula/xi.rb && git commit -m "xi 0.0.67" && git push
```
...
- `HOMEBREW_TAP_TOKEN` — a fine-grained PAT (or deploy token) with
  `contents: write` on `code-by-sia/homebrew-x`.
