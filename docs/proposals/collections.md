...
3. **`std/collections`** — **`List<T>`, `Set<T>`, and `Map<K, V>` shipped**
   ...
   as param/field/return). **Ranges, builders, and the core eager functional API shipped** (`a..b`/`until`/`downTo`/`step`; `listOf`/`setOf`/`mapOf`; `map`/`filter`/`fold`/`reduce`/`sumOf`/`count`/`any`/`all`/`none`/`forEach`/`joinToString` via inlined lambdas). More operators (sorted/distinct/take/drop/groupBy/find...) and lazy `Sequence` remain.
