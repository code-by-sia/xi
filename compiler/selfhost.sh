...
#   gen1 = compiler/xc.x compiled by gen0     (current source, seed codegen)
#   gen2 = compiler/xc.x compiled by gen1     (current source, current codegen)
#   gen3 = compiler/xc.x compiled by gen2
#
...
compile_with() {  # $1 = compiler binary, $2 = output C copy
    ( cd "$W" && XC_HELPERS="$HELP" XC_RUNTIME="$ROOT/runtime" XC_OUT=. "$1" xc.x >/dev/null )
    cp "$W/xc.gen.c" "$2"
}
...
echo "==> gen2 compiles xc.x -> gen3"
compile_with "$W/xc_gen2" "$W/c3.c"; cp "$W/xc" "$W/xc_gen3"

echo
echo "==> Fixpoint: gen2's C output for xc.x == gen3's C output?"
...
for ex in hello refined_types greeting features overload errors; do
    ...
    ( cd "$W" && XC_OUT=. XC_RUNTIME="$ROOT/runtime" ./xc_gen3 "$ex.x" >/dev/null 2>&1 )
